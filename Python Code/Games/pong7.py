...
def start_pong_game():
    ...
    while running:
        ...
        to_send = []
        add_paddle_coords(to_send, player_one_x, player_one_y, paddle_width, paddle_height, 255)
        add_ball_coords(to_send, ball_x, ball_y, ball_radius, 30, 255)
        add_paddle_coords(to_send, player_two_x, player_two_y, paddle_width, paddle_height, 255)
...
def add_paddle_coords(coords:list, paddle_x, paddle_y, paddle_width, paddle_height, colour):

    for _ in range(0, LASER_ON_DELAY):
        add_coord(coords, paddle_x, paddle_y, 0)

    points = shapes.rectangle_points(paddle_x, paddle_y, paddle_width, paddle_height, 3, 3, 20)

    for point in points:
      add_coord(coords, paddle_x + point[0], paddle_y + point[1], colour)
